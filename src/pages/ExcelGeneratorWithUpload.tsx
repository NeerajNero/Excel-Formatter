...
async function copyTextSafe(text: string) {
  ...
  // Fallback for non-secure or blocked contexts
...
} // [web:30][web:32][web:38][web:44][web:41]
...
const getSequence = (s: string) => {
  return s
    .split("")
    .map((char) => (isNaN(parseInt(char)) ? "L" : "N"))
    .join("");
}; // [web:12]
...
export default function MultiSheetBuilder() {
  ...
  const processData = () => {
    const isReadyForInvoice =
      ...
      selectedPartNumberCol &&
      selectedInvoiceCol &&
      selectedQtyCol &&
      selectedSerialCol;
    const isReadyForConsolidation =
      processingMode === "consolidate" &&
      selectedPartNumberCol &&
      selectedQtyCol &&
      selectedSerialCol;
    ...
    if (!isReadyForInvoice && !isReadyForConsolidation) {
      setNotification({
        message: "Please select all required columns for the chosen mode.",
        type: "error",
      });
      return;
    }
    ...
    setTimeout(async () => {
      try {
        ...
        const groupedData = parsedData.reduce(
          (acc: { [key: string]: DataRow[] }, row: DataRow) => {
        ...
        Object.entries(groupedData).forEach(
          ([groupKey, rows]: [string, DataRow[]]) => {
            const validRows = rows.filter((row) => Number(row[selectedQtyCol]) <= 1);
            const serials = validRows
              .map((row) => String(row[selectedSerialCol]))
              .filter(Boolean);
        ...
        // Auto-copy each created sheet (TSV with headers)
        for (const { data } of finalSheets) {
        ...
        } // [web:24][web:30][web:32][web:38]

        if (validationErrors.length > 0) {
          setMismatchedSerials(validationErrors);
          setNotification({
            message: `Processing complete with ${validationErrors.length} validation warning(s).`,
            type: "warning",
          });
        } else {
          setNotification({
            message: `${finalSheets.length} sheets created successfully. All serials validated.`,
            type: "success",
          });
        }
      } catch (error) {
  ...
  const buildParsedRowsFromManual = (entries: string[]): DataRow[] => {
    if (processingMode === "invoice") {
      if (!selectedPartNumberCol || !selectedInvoiceCol || !selectedQtyCol || !selectedSerialCol) {
        throw new Error(
          "Please map Part, Invoice/BOE, Quantity, and Serial before processing manual entries."
        );
      }
    } else {
    ...
    return entries.map((value) => {
      ...
      // For validation pipeline, serial must be present (lot-only flows still use serial col)
      row[selectedSerialCol] = String(value);
      return row;
    });
  }; // [web:17][web:6]
  ...
  const runProcessingPipeline = (sourceParsedData: DataRow[]) => {
    ...
    // Preserve current parsedData to avoid disrupting file preview
    const prev = parsedData;
    setParsedData(sourceParsedData);

    setTimeout(() => {
      try {
        processData(); // uses selected* states + parsedData
      } finally {
        // Restore original parsedData asynchronously after next tick to let processData complete
        setTimeout(() => setParsedData(prev), 0);
      }
    }, 0);
  }; // [web:17][web:45][web:47]
  ...
  const handleAddOrUpdateSheet = () => {
    ...
    const newSheet: SheetData = {
      ...
      data: buildSheetDataFromText(entries),
    };
    ...
    setRawText("");
    setSheetName("");
  }; // Raw add/edit remains (no validation)

  // New: process manual entries through the same pipeline with validation and auto-copy
  const handleProcessManual = () => {
    ...
    try {
      ...
      setNotification({
        message: err?.message || "Failed to process manual entries.",
        type: "error",
      });
    }
  }; // [web:17]
  ...
  const handleExport = () => {
    ...
    const summaryData = sheetDataList.map((s) => ({
      "Sheet Name": s.sheetName,
      "Serial Count": s.data.length,
    }));
    ...
    setNotification({
      message: "Excel file with summary has been exported!",
      type: "success",
    });
  }; // [web:4][web:37]
  ...
  const handleCopySheet = async (index: number) => {
    ...
    const headerString = headers.join("\t");
    const rowsString = sheet.data
      .map((row) => headers.map((h) => row[h as keyof RowData] ?? "").join("\t"))
      .join("\n");
    ...
    try {
      await copyTextSafe(clipboardText);
      setNotification({
        message: `Copied data for "${sheet.sheetName}" to clipboard.`,
        type: "success",
      });
    } catch (err) {
  ...
  const handleEditSheet = (index: number) => {
    ...
    setRawText(
      sheet.data
        .map((row) => (isLot ? row["Lot No."] : row["Serial No."]))
        .join("\n")
    );
    ...
    <div className="container py-5">
      {notification && (
        ...
        >
          ...
          {mismatchedSerials.length > 0 && (
            <button
              className="btn btn-sm btn-light ms-3"
              onClick={() => setShowMismatchedModal(true)}
            >
              Show Details
            </button>
          )}
            ...
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button
                  type="button"
                  className="btn-close"
                  onClick={() => setShowMismatchedModal(false)}
                ></button>
              </div>
              <div className="modal-body">
                <table className="table table-sm table-striped">
                  <thead>
                    <tr>
                      <th>Group</th>
                      <th>Mismatched Serial</th>
                      <th>Reason</th>
                    </tr>
                  </thead>
                  <tbody>
                    {mismatchedSerials.map((e, i) => (
                      <tr key={i}>
                        <td>{e.group}</td>
                        <td>{e.serial}</td>
                        <td>{e.reason}</td>
                  ...
                  <div className="table-responsive" style={{ maxHeight: "150px" }}>
                    <table className="table table-sm table-bordered table-striped">
                      <thead className="table-dark">
                        <tr>{fileHeaders.map((h, i) => <th key={i}>{h}</th>)}</tr>
                      </thead>
                      <tbody>
                ...
                <div className="card bg-light p-3 mt-3">
                  ...
                  <div className="form-check">
                    <input
                      ...
                      name="mode"
                      id="invoiceMode"
                      checked={processingMode === "invoice"}
                      onChange={() => setProcessingMode("invoice")}
                    />
                    <label className="form-check-label fw-bold" htmlFor="invoiceMode">
                      Separate by Part Number & Invoice/BOE
                    </label>
                  </div>
                  <div className="form-check mb-3">
                    <input
                      ...
                      name="mode"
                      id="consolidateMode"
                      checked={processingMode === "consolidate"}
                      onChange={() => setProcessingMode("consolidate")}
                    />
                    <label className="form-check-label fw-bold" htmlFor="consolidateMode">
                      Consolidate all Serials by Part Number only
                    </label>
                  </div>
                  ...
                  <div className="d-flex justify-content-between align-items-center mt-2">
                    ...
                    <div>
                      <button className="btn btn-sm btn-outline-secondary me-2" onClick={() => saveMapping()}>
                        Save
                      </button>
                      <button className="btn btn-sm btn-outline-secondary" onClick={() => loadMapping(fileHeaders)}>
                        Load
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="row g-3 my-2">
                    <div className="col-md-6">
                      ...
                      <select
                        className="form-select"
                        value={selectedPartNumberCol}
                        onChange={(e) => setSelectedPartNumberCol(e.target.value)}
                      >
                        <option value="" disabled>
                          -- Select --
                        </option>
                        {fileHeaders.map((h, i) => (
                          <option key={`part-${i}`} value={h}>
                            {h}
                          </option>
                        ))}
                      </select>
                    ...
                    {processingMode === "invoice" && (
                      <div className="col-md-6">
                        ...
                        <select
                          className="form-select"
                          value={selectedInvoiceCol}
                          onChange={(e) => setSelectedInvoiceCol(e.target.value)}
                        >
                          <option value="" disabled>
                            -- Select --
                          </option>
                          {fileHeaders.map((h, i) => (
                            <option key={`inv-${i}`} value={h}>
                              {h}
                            </option>
                          ))}
                        </select>
                    ...
                    <div className="col-md-6">
                      ...
                      <select
                        className="form-select"
                        value={selectedSerialCol}
                        onChange={(e) => setSelectedSerialCol(e.target.value)}
                      >
                        <option value="" disabled>
                          -- Select --
                        </option>
                        {fileHeaders.map((h, i) => (
                          <option key={`sn-${i}`} value={h}>
                            {h}
                          </option>
                        ))}
                      </select>
                  ...
                  <div className="form-check form-switch mb-3">
                    <input
                      ...
                      type="checkbox"
                      role="switch"
                      id="validateSerialsSwitch"
                      checked={validateSerials}
                      onChange={(e) => setValidateSerials(e.target.checked)}
                    />
                    <label className="form-check-label fw-bold" htmlFor="validateSerialsSwitch">
                      Validate serial number sequence and length
                    </label>
                  </div>
              ...
              <div className="card shadow">
                <div className="card-body">
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <h2 className="card-title mb-0">Generated Sheets</h2>
                    <button className="btn btn-success" onClick={handleExport}>
                      📤 Export All w/ Summary
                    </button>
                  </div>
                    ...
                    {sheetDataList.map((s, i) => (
                      ...
                      >
                        ...
                        <div className="btn-group">
                          <button className="btn btn-sm btn-outline-secondary" onClick={() => handleCopySheet(i)}>
                            📋 Copy
                          </button>
                          <button className="btn btn-sm btn-outline-primary" onClick={() => handleEditSheet(i)}>
                            ✏️ Edit
                          </button>
                          <button className="btn btn-sm btn-outline-danger" onClick={() => handleDeleteSheet(i)}>
                            🗑️ Delete
                          </button>
                        </div>
                      </li>
            ...
            <div id="manual-builder" className="card shadow">
              <div className="card-body">
                ...
                <p className="text-muted">
                  Paste data to add or process with validation and auto-copy.
                </p>
                ...
                <div className="row g-3 mb-3">
                  <div className="col-md-6">
                    <label className="form-label fw-bold">Sheet/Part Name</label>
                    <input
                      className="form-control"
                      type="text"
                      value={sheetName}
                      onChange={(e) => setSheetName(e.target.value)}
                      placeholder="Optional (used as Part Number in manual processing)"
                    />
                  </div>
                  <div className="col-md-6">
                ...
                <div className="d-flex gap-2">
                  <button className="btn btn-primary w-100" onClick={handleProcessManual}>
                    ⚙️ Process Manual (validated)
                  </button>
                  <button className="btn btn-outline-primary w-100" onClick={handleAddOrUpdateSheet}>
                    {editingSheetIndex !== null ? "💾 Update Sheet (raw)" : "➕ Add Sheet (raw)"}
                  </button>
